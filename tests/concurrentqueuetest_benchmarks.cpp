...
void PrintResult(const Result& r, std::size_t totalItems) {
    std::cout << "[" << r.name << "] time=" << r.seconds << "s  throughput="
              << r.throughput << " items/s"
...
Result TestFastQueue_EnqDeq(const BenchmarkConfig& cfg) {
    hakle::HakleFlagsBlockManager<int, 32> block_manager(32);
    hakle::FastQueue<int, 32> queue(32, block_manager);
...
void PrintRanking(const std::vector<Result>& results) {
    ...
    for (const auto& r : sorted) {
        ...
        std::cout << std::left << std::setw(36) << r.name
                  << " | " << std::setw(barWidth) << std::string(barLen, '#')
...
int main() {
    ...
    std::cout << "Benchmark: prodThreads=" << cfg.prodThreads
              << " consThreads=" << cfg.consThreads
              << " itemsPerProd=" << cfg.itemsPerProd
              << " totalItems=" << totalItems << "\n\n";

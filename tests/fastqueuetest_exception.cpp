...
using Block        = hakle_flags_block<ThrowOnCtor, kBlockSize>;
using BlockManager = hakle_block_manager<Block>;
using Queue        = fast_queue<ThrowOnCtor, kBlockSize>;
...
TEST( ExplicitQueueExceptionTest, EnqueueBulk_ExceptionRollback ) {
    ...
    bool caught = false;
    try {
        q.enqueue_bulk<alloc_mode::can_alloc>( src.begin(), COUNT );
    }
    catch ( std::exception const& ) {
        caught = true;
    }
...
TEST( ExplicitQueueExceptionTest, EnqueueBulk_FailThenSuccess ) {
    ...
    // 第二次：不抛异常，应该完全成功
    ThrowOnCtor::Reset( /*throwOn=*/-1 );
    EXPECT_NO_THROW( q.enqueue_bulk<alloc_mode::can_alloc>( src.begin(), COUNT ) );
    EXPECT_EQ( q.size(), COUNT );

    // 全部 Dequeue 出来
    std::vector<ThrowOnCtor>* out = new std::vector<ThrowOnCtor>( COUNT );
    auto                      got = q.dequeue_bulk( out->begin(), COUNT );
    EXPECT_EQ( got, COUNT );
    EXPECT_EQ( q.size(), 0u );

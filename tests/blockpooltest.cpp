...
// 测试用的基础块类型
struct TestBlock : public BaseBlock<int, 64> {
    // 可以添加额外的成员变量或方法
    int custom_data{0};
};

// 自定义块类型用于测试继承约束
struct CustomBlock : public BaseBlock<double, 32> {
...
TEST_F(BlockPoolTest, BasicBlockAllocation) {
    ...
    for (size_t i = 0; i < POOL_SIZE; ++i) {
        ...
        EXPECT_EQ(block->Elements.size(), BLOCK_SIZE);
    }
...
TEST_F(BlockPoolTest, ElementAccess) {
    ...
    for (size_t i = 0; i < BLOCK_SIZE; ++i) {
        block->Elements[i] = static_cast<int>(i * 10);
    }

    // 验证元素值
    for (size_t i = 0; i < BLOCK_SIZE; ++i) {
        EXPECT_EQ(block->Elements[i], static_cast<int>(i * 10));
    }
}
...
TEST_F(BlockPoolTest, CacheLineAlignment) {
    ...
    EXPECT_EQ(addr1 % HAKLE_CACHE_LINE_SIZE, 0);
    EXPECT_EQ(addr2 % HAKLE_CACHE_LINE_SIZE, 0);

    // 块之间应该有足够的间距（至少缓存行大小）
    auto distance = std::abs(static_cast<long>(addr2 - addr1));
    EXPECT_GE(distance, sizeof(BaseBlock<int, BLOCK_SIZE>));
}
...
TEST_F(BlockPoolTest, ThreadSafety) {
    ...
    // 用于记录分配的块，确保没有重复分配
    std::array<std::atomic<BaseBlock<int, BLOCK_SIZE>*>, POOL_SIZE> allocated_blocks;
        ...
        for (size_t i = 0; i < BLOCKS_PER_THREAD; ++i) {
            ...
            if (block != nullptr) {
                ...
                for (auto& allocated : allocated_blocks) {
                    BaseBlock<int, BLOCK_SIZE>* expected = nullptr;
                    if (allocated.compare_exchange_strong(expected, block,
                ...
                for (auto& element : block->Elements) {
                    element = thread_id * 1000 + static_cast<int>(i);
                }
    ...
    std::cout << "Thread safety test: " << successful_allocations.load()
              << " successful, " << failed_allocations.load() << " failed" << std::endl;
}
...
TEST_F(BlockPoolTest, SingleBlockPool) {
    ...
    // 验证块可用
    block1->Elements[0] = 42;
    EXPECT_EQ(block1->Elements[0], 42);
}
...
TEST_F(BlockPoolTest, MemoryLayout) {
    ...
    for (size_t i = 1; i < blocks.size(); ++i) {
        ...
        // 块应该在内存中连续（或至少顺序正确）
        EXPECT_LT(reinterpret_cast<uintptr_t>(prev),
...
TEST_F(BlockPoolTest, TypeConstraints) {
    // 应该能正常编译（正确继承）
    struct GoodBlock : public BaseBlock<float, 16> {
        // 可以添加自定义成员
    };
...
TEST_F(BlockPoolTest, PerformanceLargePool) {
    ...
    for (size_t i = 0; i < POOL_SIZE; ++i) {
        ...
        // 简单操作
        block->Elements[0] = static_cast<int>(i);
    }

    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);

    std::cout << "Allocated " << POOL_SIZE << " blocks in "

...
std::uint64_t CalcExpectedSum(std::size_t prodThreads,
                              std::size_t itemsPerProd)
{
    std::uint64_t sum = 0;
...
TEST(ConcurrentQueueCorrectness, MultiProducerMultiConsumer_NormalEnqDeq)
{
    ...
    const std::uint64_t expectedSum =
        CalcExpectedSum(prodThreads, itemsPerProd);
        ...
        consumers.emplace_back([&] {
            ...
            while (consumed.load(std::memory_order_relaxed) < totalItems) {
                if (queue.TryDequeue(value)) {
                    sum.fetch_add(static_cast<std::uint64_t>(value),
...
TEST(ConcurrentQueueCorrectness, ProducerToken_Enq_TryDequeueFromProducer)
{
    ...
    for (std::size_t c = 0; c < consThreads; ++c) {
        consumers.emplace_back([&] {
            ...
            while (consumed.load(std::memory_order_relaxed) < totalItems) {
                std::size_t idx =
                    nextProducer.fetch_add(1, std::memory_order_relaxed)
                    % prodThreads;

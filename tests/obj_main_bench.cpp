...
BENCHMARK( BM_CQ_NormalEnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_CQ_BulkEnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
    ...
    for ( auto _ : state ) {
        ...
        for ( std::size_t c = 0; c < kConsThreads; ++c ) {
            consumers.emplace_back( [ & ] {
                Obj value;
                while ( consumed.load( std::memory_order_relaxed ) < totalItems ) {
                    std::size_t idx = nextProducerForConsumer.fetch_add( 1, std::memory_order_relaxed ) % kProdThreads;
...
BENCHMARK( BM_CQ_ProdToken_EnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
    ...
    for ( auto _ : state ) {
        ...
        for ( std::size_t c = 0; c < kConsThreads; ++c ) {
            consumers.emplace_back( [ & ] {
                ...
                while ( consumed.load( std::memory_order_relaxed ) < totalItems ) {
                    std::size_t idx = nextProducerForConsumer.fetch_add( 1, std::memory_order_relaxed ) % kProdThreads;
                    std::size_t got = queue.TryDequeueBulkFromProducer( prodTokens[ idx ], buf.data(), kBulkSize );
                    if ( got > 0 ) {
                        consumed.fetch_add( got, std::memory_order_relaxed );
                    }
                }
...
BENCHMARK( BM_CQ_ProdToken_BulkEnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_CQ_ProdTokenEnq_ConsTokenDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_CQ_NormalEnq_ConsTokenDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_CQ_ProdTokenBulkEnq_ConsTokenBulkDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
static void BM_MOODY_NormalEnqDeq( benchmark::State& state ) {
    for ( auto _ : state ) {
        moodycamel::ConcurrentQueue<Obj> queue;
        const std::size_t                totalItems = kProdThreads * kItemsPerProd;
...
BENCHMARK( BM_MOODY_NormalEnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_MOODY_BulkEnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_MOODY_ProdToken_EnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
    ...
    for ( auto _ : state ) {
        ...
        for ( std::size_t c = 0; c < kConsThreads; ++c ) {
            consumers.emplace_back( [ & ] {
                ...
                while ( consumed.load( std::memory_order_relaxed ) < totalItems ) {
                    std::size_t idx = nextProducerForConsumer.fetch_add( 1, std::memory_order_relaxed ) % kProdThreads;
                    std::size_t got = queue.try_dequeue_bulk_from_producer( prodTokens[ idx ], buf.data(), kBulkSize );
                    if ( got > 0 ) {
                        consumed.fetch_add( got, std::memory_order_relaxed );
                    }
                }
...
BENCHMARK( BM_MOODY_ProdToken_BulkEnqDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_MOODY_ProdTokenEnq_ConsTokenDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_MOODY_NormalEnq_ConsTokenDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_MOODY_ProdTokenBulkEnq_ConsTokenBulkDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );
...
BENCHMARK( BM_MOODY_NormalBulkEnq_ConsTokenBulkDeq )->MeasureProcessCPUTime()->Iterations( 10 )->Unit( benchmark::kMillisecond );

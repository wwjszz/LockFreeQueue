...
#else
template <class T, typename std::enable_if<std::is_integral<T>::value || std::is_floating_point<T>::value || std::is_pointer<T>::value, int>::type =
              0>
#endif
class WeakAtomic {
public:
    ...
    WeakAtomic( const WeakAtomic& Other ) noexcept(std::is_nothrow_copy_constructible<T>::value)
        : Value( Other.Load() ) {
    }

    WeakAtomic( WeakAtomic&& Other ) noexcept
...
#endif
    explicit WeakAtomic( U&& Other ) noexcept(std::is_nothrow_constructible<T, U>::value)
    ...
    T FetchAdd( T Increment ) noexcept {
        return Value.fetch_add( Increment, std::memory_order_relaxed );
    }

    T FetchAddRelease( T Increment ) noexcept {
        return Value.fetch_add( Increment, std::memory_order_release );
    }

    T FetchAddAcquire( T Increment ) noexcept {
...
public:
    ...
    ReaderWriterQueue( ReaderWriterQueue&& Other ) noexcept
        : FrontBlock( Other.FrontBlock ), TailBlock( Other.TailBlock ),
    ...
    [[nodiscard]] bool TryEnqueue( const T& Item ) {
        return InnerEnqueue<AllocMode::CannotAlloc>( Item );
    }

    [[nodiscard]] bool TryEnqueue( T&& Item ) {
        return InnerEnqueue<AllocMode::CannotAlloc>( std::move( Item ) );
    }
    ...
    bool Enqueue( const T& Item ) {
        return InnerEnqueue<AllocMode::CanAlloc>( Item );
    }

    bool Enqueue( T&& Item ) {
        return InnerEnqueue<AllocMode::CanAlloc>( std::move( Item ) );
    }
...
private:
    ...
    struct QueueGuard {
        explicit
        QueueGuard( WeakAtomic<bool>& InStatus ) noexcept
            : Status( InStatus ) {
            assert(
                !InStatus &&
    ...
    struct Block {
        ...
        Block( const std::size_t Size, char* InRawThis, char* InRawData, T* InData ) noexcept
            : Front( 0 ), LocalTail( 0 ), Tail( 0 ),
              LocalFront( 0 ), Next( nullptr ),
              Data( InData ), SizeMask( Size - 1 ), RawThis( InRawThis ), RawData( InRawData ) {
        }
#endif
    ...
    static Block* MakeBlock( std::size_t capacity ) noexcept {
        ...
        char* NewBlock = OPERATOR_NEW( char, sizeof(Block) + alignof(Block) - 1 );
        if (!NewData || !NewBlock) {
            return nullptr;
        }
...
public:
    ...
    explicit LightWeightSemaphore( signed_size_t InitialCount = 0 )
        : Count( InitialCount ), Sem( 0 ) {
    ...
    bool Wait( const std::int64_t Timeout = -1 ) {
        return TryWait() || TryWaitWithPartialSpinning( Timeout );
    }

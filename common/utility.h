...
inline constexpr bool circular_less_than( T a, T b ) noexcept {
    return static_cast<T>( a - b ) > static_cast<T>( static_cast<T>( 1 ) << ( static_cast<T>( sizeof( T ) * CHAR_BIT - 1 ) ) );
}

inline std::size_t ceil_to_pow2( std::size_t X ) noexcept {
    --X;
    X |= X >> 1;
    X |= X >> 2;
    X |= X >> 4;
...
inline constexpr uint8_t bit_width( std::size_t X ) noexcept {
    uint8_t Count = 0;
